...
// Re-hashes the 20-byte address currently stored at pInverse[id].d[0..4]
// with keccak256 and writes the FIRST 20 bytes of the hash back to the same
// location so the existing score kernels match against bytes of
// keccak256(address) instead of the address itself.
//
// Padding: Ethereum-style keccak256 uses delimiter 0x01 (not NIST SHA-3's 0x06).
// The trailing 0x80 at byte 135 is XOR'd inside sha3_keccakf itself
// (h->d[33] ^= 0x80000000), so we only need to XOR 0x01 at byte 20.
__kernel void profanity_transform_keccak_address(__global mp_number * const pInverse) {
	const size_t id = get_global_id(0);
	...
	h.b[20] ^= 0x01; // keccak256 padding delimiter

	sha3_keccakf(&h);

	// Store FIRST 20 bytes of keccak256(address). Score kernels read hash[0..19]
	// so data2[0..N-1] = 0 with data1[0..N-1] = 0xFF matches keccak256(addr)[0..N-1].
	pInverse[id].d[0] = h.d[0];
	pInverse[id].d[1] = h.d[1];
	pInverse[id].d[2] = h.d[2];
	pInverse[id].d[3] = h.d[3];
	pInverse[id].d[4] = h.d[4];
}
