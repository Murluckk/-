...
int main(int argc, char * * argv) {
	...
	try {
		...
		argp.addSwitch('k', "keccak-address", bMineKeccakAddress);
		argp.addSwitch('Q', "quit-score", quitScore);
		...
		if (bMineContract && bMineKeccakAddress) {
			std::cout << "error: --contract and --keccak-address are mutually exclusive" << std::endl;
			return 1;
		}
		if (bMineContract) {
			mode.target = CONTRACT;
		} else if (bMineKeccakAddress) {
			mode.target = ADDRESS_KECCAK;
		} else {
			mode.target = ADDRESS;
		}
		...
		Dispatcher d(clContext, clProgram, mode, worksizeMax == 0 ? inverseSize * inverseMultiple : worksizeMax, inverseSize, inverseMultiple, static_cast<cl_uchar>(quitScore), strPublicKey);
		for (auto & i : vDevices) {
